...
import type { DepartamentoResponse, EditarDepartamentoRequest } from '@/types'
...
const form = ref<EditarDepartamentoRequest>({
...
async function guardar() {
  ...
  error.value = null
  try {
    await api.put(`/departamento/${props.departamento.id}/editar`, form.value)
    emit('editado')
  } catch (err: any) {
    error.value = err.response?.data?.error ?? 'No se pudo editar el departamento.'
  } finally {
    ...
    <form class="modal" @submit.prevent="guardar">
      <div class="modal-header">
        <div>
          ...
          <p class="subtitulo">{{ departamento.nombre }}</p>
        </div>
